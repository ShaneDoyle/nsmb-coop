...
u32 MegaDestructionCounter_tempVar;

Actor* ActorFixes_megaRewardSpawnActor(u16 objectID, u32 settings, const Vec3* position, const Vec3s* rotation, const fx32* scale, const s8* linkPlayerID)
{
	return Actor::spawnActor(objectID, settings | (MegaDestructionCounter_tempVar << 16), position, rotation, scale, linkPlayerID);
}
...
ncp_set_call(0x020D0D3C, 10, ActorFixes_megaRewardSpawnActor)
...
// Mega Mushroom Destruction Counter ----------------------------------------------------------------------------

bool MegaDestructionCounter_someoneHasMegaTimer()
{
...
bool MegaDestructionCounter_someoneHasMegaPowerup()
{
	for (u32 playerID = 0; playerID < Game::getPlayerCount(); playerID++)
	{
...
bool MegaDestructionCounter_everyoneMegaDamaged()
{
	for (u32 playerID = 0; playerID < Game::getPlayerCount(); playerID++)
	{
		Player* player = Game::getPlayer(playerID);
...
ncp_set_call(0x0215F858, 54, MegaDestructionCounter_someoneHasMegaTimer)
ncp_set_call(0x0215F98C, 54, MegaDestructionCounter_someoneHasMegaTimer)

ncp_set_call(0x0215F834, 54, MegaDestructionCounter_someoneHasMegaPowerup)
ncp_repl(0x0215F838, 54, "NOP; NOP; CMP R0, #1")

ncp_set_call(0x0215F7EC, 54, MegaDestructionCounter_everyoneMegaDamaged)
ncp_repl(0x0215F7F0, 54, "NOP; CMP R0, #1")

ncp_set_call(0x0215F998, 54, MegaDestructionCounter_everyoneMegaDamaged)
ncp_repl(0x0215F99C, 54, "NOP; CMP R0, #1")

ncp_call(0x0215F660, 54)
Player* MegaDestructionCounter_getLevelEndRewardTargetPlayer()
{
	return Game::getPlayer(MegaDestructionCounter_tempVar);
}

ncp_set_call(0x0215F61C, 54, ActorFixes_megaRewardSpawnActor)

/*
extern "C" {
	void MegaDestructionCounter_fun0215FA0C(u32 self, u32);
	extern fx32 Stage_zoom[2];
}
...
void MegaDestructionCounter_spawnRewards(u32 self)
{
	...
	{
		MegaDestructionCounter_fun0215FA0C(self, 2);
		return;
	}
	...
	for (u32 playerID = 0; playerID < Game::getPlayerCount(); playerID++)
	{
		if (Game::getPlayerDead(playerID) || Stage_playerInFlagpoleSentFlying[playerID])
			continue;

		fx32 cameraY = Stage::cameraY[playerID];
		fx32 cameraX = Stage::cameraX[playerID];
		fx32 zoom = Stage_zoom[playerID];
		...
		MegaDestructionCounter_tempVar = playerID; // Spawned reward must track the correct player
...
ncp_asmfunc void MegaDestructionCounter_spawnRewards_ASM()
{asm(R"(
	...
	b	MegaDestructionCounter_fun0215FA0C
.mdcsi_L30:
	add	r6, r6, #1
	cmp	r6, #5
	bne	.mdcsi_L28
...
.mdcsi_L32:
	...
	b	.mdcsi_L31
.mdcsi_L40:
	.word	Stage_playerInFlagpoleSentFlying
	.word	_ZN5Stage7cameraYE
	.word	_ZN5Stage7cameraXE
	.word	_ZN5Stage4zoomE
	.word	MegaDestructionCounter_tempVar
)");}
