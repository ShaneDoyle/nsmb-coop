...
extern "C"
{
	void CoopSym_debug_printf(const u16 colors[2], u16* dst, const char* str, ...);
	void CoopSym_debug_clear();
	void CoopSym_debug_drawTop();
	void CoopSym_debug_drawBottom();
}
...
{
	CoopSym_debug_drawTop();
	CoopSym_debug_drawBottom();
}
...
ncp_thumb void update()
{
	...
	for (u32 i = 0; i < sizeof(textEntries) / sizeof(TextEntry); i++)
	{
		...
		CoopSym_debug_printf(colors[e.color], rcast<u16*>(rcast<u8*>(screens[e.screen]) + e.pos), e.text);
	}

	char* buildTime = rcast<char*>(0x02088BB4);
	CoopSym_debug_printf(colors[COLOR_YELLOW], SCR_POS(screens[1], 1, 22), buildTime);
}
