...
extern "C" {
	...
	void nFS_LoadFileByIDToDest(u32 id, u8* dest);

	// Writes a file by ID to the destination (only writes the specified size)
	void nFS_LoadFileByIDToDestWithSize(u32 id, u8* dest, u32 size);
	
	// Writes a file by path to the destination (only writes the specified size)
	void nFS_LoadFileByNameToDestWithSize(const char* path, u8* dest, u32 size);
	...
	void nFS_LoadArchiveByID(u32 id, void* destHeap);
